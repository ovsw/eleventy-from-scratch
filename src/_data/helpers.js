module.exports = {
    ...
    getSiblingContent(collection, item, limit = 3, random = true) {
      ...
      if (random) {
        let currentItemIndex = filteredItems.length;

        while (currentItemIndex > 0) {
          // Pick a random index
          let randomIndex = Math.floor(Math.random() * currentItemIndex);

          currentItemIndex--;

          let temp = filteredItems[currentItemIndex];

          // Swap the last element with the random one
          filteredItems[currentItemIndex] = filteredItems[randomIndex];
          filteredItems[randomIndex] = temp;
        }
      }
